            ...
            [
                [
                    InlineKeyboardButton(
                        "sᴜᴘᴘᴏʀᴛ 😊", url="https://t.me/SSK_MOVIES"
                    )
                ]
            ]
        ),
    ...
    if message.chat.id == SUPPORT_GROUP :
                ...
                if total != 0:
                    ...
                    msg = await message.reply_text(script.SUPPORT_GRP_MOVIE_TEXT.format(message.from_user.mention() , total) ,             reply_markup=InlineKeyboardMarkup([
                        [ InlineKeyboardButton('ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ 😉' , url="https://t.me/SSK_MOVIES")]
                        ]))
...
async def next_page(bot, query):
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"📁 {get_size(file.file_size)}≽ {formate_file_name(file.file_name)}", url=f'https://t.me/{temp.U_NAME}?start=file_{query.message.chat.id}_{file.file_id}'),]
                for file in files
              ]
    btn.insert(0,[
        InlineKeyboardButton("✨ ɢᴇᴛ ᴀʟʟ ғɪʟᴇs ✨", callback_data=batch_link),
        InlineKeyboardButton("♻️ʟᴀɴɢᴜᴀɢᴇ ♻️", callback_data=f"languages#{key}#{offset}#{req}"),
        ])
    btn.insert(1, [
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [[
                            ...
                            [
                             InlineKeyboardButton('Iɴʟɪɴᴇ Sᴇᴀʀᴄʜ ☌', switch_inline_query_current_chat='')
                                ...
                                InlineKeyboardButton('🌐 ᴡᴇʙsɪᴛᴇ 🌐', url=https://ynvmovies.xyz)
                            ]]    
        ...
        if user != query.from_user.id:
            await query.answer(script.ALRT_TXT, show_alert=True)
            return
        link = f"https://t.me/{temp.U_NAME}?start=allfiles_{group_id}-{message_id}"
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    ...
    if settings["link"]:
        ...
        btn = [[InlineKeyboardButton(text=f"🔗 {get_size(file.file_size)}≽ {formate_file_name(file.file_name)}", url=f'https://t.me/{temp.U_NAME}?start=file_{message.chat.id}_{file.file_id}'),]
               for file in files
              ]
    if offset != "":
